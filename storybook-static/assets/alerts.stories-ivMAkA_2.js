const{useEffect:d}=__STORYBOOK_MODULE_PREVIEW_API__,g={title:"Feedback/Alerts & Dialogs"},e=`
...
`.trim(),t=()=>(d(()=>{const s=document.getElementById("btn-success-alert"),r=document.getElementById("btn-warning-confirm"),n=document.getElementById("btn-trigger-toast");s&&(s.onclick=()=>{window.CitruSS.fire({title:"Operation Successful",text:"The glassmorphic container compiled successfully without any visual degradation.",icon:"success"})}),r&&(r.onclick=()=>{window.CitruSS.confirm("Delete System Logs","Are you absolutely sure you want to purge the database logs?").then(l=>{l.isConfirmed&&window.CitruSS.toast({title:"Purged",message:"Database logs purged successfully.",type:"success"})})}),n&&(n.onclick=()=>{window.CitruSS.toast({title:"System Alert",message:"A micro-animation took place inside the GPU cache layer.",type:"info"})})},[]),`
      ...
      <details class="citruss-code-wrapper" style="margin-top: 32px; border: 1px solid var(--citruss-glass-border); border-radius: 12px; background: rgba(0,0,0,0.2); overflow: hidden; font-family: 'Outfit', sans-serif;">
        ...
        <div style="padding: 16px; border-top: 1px solid var(--citruss-glass-border); background: var(--citruss-bg-surface); overflow-x: auto; display: flex; flex-direction: column; gap: 16px;">
          <div>
            ...
            <pre style="margin: 0; white-space: pre-wrap;"><code style="font-family: monospace; color: var(--citruss-text-main); font-size: 0.8rem; line-height: 1.5;">${e.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</code></pre>
          </div>
          <div>
  ...
  `);t.parameters={docs:{source:{code:`${e}

/* Programmatic JS Usage */
${c}`}}};var i,a,o;t.parameters={...t.parameters,docs:{...(i=t.parameters)==null?void 0:i.docs,source:{originalSource:`() => {
  useEffect(() => {
    ...
    if (btnWarningConfirm) {
      btnWarningConfirm.onclick = () => {
        window.CitruSS.confirm('Delete System Logs', 'Are you absolutely sure you want to purge the database logs?').then(res => {
          if (res.isConfirmed) {
            window.CitruSS.toast({
              title: 'Purged',
              message: 'Database logs purged successfully.',
  ...
  \`;
}`,...(o=(a=t.parameters)==null?void 0:a.docs)==null?void 0:o.source}}};const u=["DialogsAndToasts"];export{t as DialogsAndToasts,u as __namedExportsOrder,g as default};
